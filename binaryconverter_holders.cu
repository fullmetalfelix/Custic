...
int main (int argc, char* argv[])
{
  ...
  char filename[100], nameout[100],hldname[100],aname[5];
  ...
  sprintf(hldname , "%s.loop%i.hld%i.xyz",argv[1],loop,img);
  ...
  while(!feof(fin)){
	...
	break;
      ...
      fprintf(fhld,"%s ",aname);
      
      for(j=0;j<nimages;j++){
      
	fread(&pos,sizeof(short3),1,fin);  //->position as short3
	...
	fread(&pos,sizeof(short3),1,fin);  //->holder as short3
	if(j==img-1){
	  speed.x = ((float)pos.x)/100.0f;
	  speed.y = ((float)pos.y)/100.0f;
	  speed.z = ((float)pos.z)/100.0f;
